...
#define USING_PRIORITY_QUEUE false
#define VERBOSE true
...
public:
	...
	static bool usingPriorityQueue;
	...
	virtual int printReceiver() { return 0;};
	//Prints out during the receiver printout step (after the simulation)
	...
	virtual int getSumPacketDelay() {return 0; }
	
	virtual Queue<double> averageResponseTimeByPacketSize()
	{
		Queue<double> x;
		x.enqueue(0); x.enqueue(0); x.enqueue(0);
		x.enqueue(0); x.enqueue(0); x.enqueue(0);
		return x;
	}
};
...
public:
	...
	//Initializes the values for source. Must be called separately from the constructor
...
class Mule : public Node
{
	...
	Queue<Packet> queueMid;
	Queue<Packet> queueLarge;
...
class Receiver : public Node
{
	...
	friend int printReceivers(Node **n, int sumPackets );
...
public:
	...
	virtual int printReceiver();		//Prints out this receivers data and analysis
	
	int queueLength() const;			//Returns the length of the queue (used for debugging)
	
	virtual int getSumPacketDelay();
...
public:
	...
	//If the default argument is present, it won't simply use the node in the given position.
