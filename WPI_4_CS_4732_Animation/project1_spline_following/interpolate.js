...
function catmullRomSpline(plast,pstart,pend,pnext, u) {
		...
		var pos = [0,0,0,0], vec = [];
		vec4.scale(vec, plast, -0.5*u+u2-0.5*u3);
		vec4.add(pos, pos, vec);
		vec4.scale(vec, pstart, 1+ 0 -2.5*u2 + 1.5*u3);
		vec4.add(pos,pos,vec);
		vec4.scale(vec, pend, 0+0.5*u+2*u2-1.5*u3);
		vec4.add(pos,pos,vec);
		vec4.scale(vec, pnext, 0+0-0.5*u2 + 0.5*u3);
		vec4.add(pos,pos,vec);
		return pos;
}
