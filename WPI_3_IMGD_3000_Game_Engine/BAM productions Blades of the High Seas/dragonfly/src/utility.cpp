...
df::Position df::viewToWorld(Position view_pos) {
	Position world_origin = WorldManager::getInstance().getView().getCorner();//grab the boundary box.
	int world_x = world_origin.getX();
	int world_y = world_origin.getY();
	Position world_pos(view_pos.getX() + world_x, view_pos.getY() + world_y);
	return world_pos;
}
