...
GLuint makeProgram() {
	return InitShader( ".\\Shader Files\\vshader1.glsl", ".\\Shader Files\\fshader1.glsl" );
}
...
void setMaterial(color4 diffuse, color4 specular, color4 ambient, GLfloat shininess, GLuint program) {
	...
	//Inputs should really be broken up into absorbtion coefficient & incoming intensity...


	GLuint DiffuseProduct = glGetUniformLocationARB(program, "DiffuseProduct");
	glUniform4fv( DiffuseProduct, 1, diffuse );

	GLuint SpecularProduct = glGetUniformLocationARB(program, "SpecularProduct");
	glUniform4fv( SpecularProduct, 1, specular );

	GLuint AmbientProduct = glGetUniformLocationARB(program, "AmbientProduct");
...
void setLight(mat4 ctm,vec4 position, GLuint program) {
	...
	glUniform4fv(LightPosition, 1, ctm * position);
}